...
int get_cols();
int get_lines();
int set_geschwindigkeit(int eingabe);
void special_point( pthread_t th1, pthread_t th2);
int del_special_point(struct koordinaten *point);
...
int del_special_point(struct koordinaten *point) {
  ...
  if(zeiger->xachse>point->x) {
    len+=(zeiger->xachse-point->x);
  }
  else if(zeiger->xachse<=point->x) {
    len+=(point->x-zeiger->xachse);
  }

  if(zeiger->yachse>point->y) {
    len+=(zeiger->yachse-point->y);
  }
  else if(zeiger->yachse<=point->y) {
    len+=(point->y-zeiger->yachse);
  }
  len+=10; 
  len*=geschwindigkeit;
...
void special_point( pthread_t th1, pthread_t th2) {
  ...
  if (pthread_create (&th1,NULL,&del_special_point,&arg) != 0) {
